...
import { createOldStudent } from "../_lib/data-service";
...
const validationSchema = Yup.object({
  ...
  GradePercentage: Yup.number()
    .required("Required")
    .min(0)
    .max(100, "Percentage must be between 0 and 100"),
  PreviousYearHosting: Yup.boolean().required("Required"),
  PreviousYearGrade: Yup.number().required("Required"),
...
function ReservationFormOld({ cabin, country, governorates, colleges }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      console.log(values);
      try {
        const result = await createOldStudent(values, roomId);
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            <div className="flex-1 min-w-[200px]">
              <label
                htmlFor="PreviousYearGrade"
                className="block text-sm font-medium"
              >
                {mounted ? t("room.form.previousYearGrade") : "Loading..."}
              </label>
              <select
                id="PreviousYearGrade"
                name="PreviousYearGrade"
                onChange={(e) =>
                  formik.setFieldValue(
                    "PreviousYearGrade",
                    Number(e.target.value)
                ...
                value={formik.values.PreviousYearGrade}
                className="w-full p-3 border bg-primary-200 text-primary-800 shadow-sm rounded-md"
              >
              ...
              {formik.errors.PreviousYearGrade &&
                formik.touched.PreviousYearGrade && (
                  <p className="text-red-500 text-xs">
                    {formik.errors.PreviousYearGrade}
                  </p>
                )}
            ...
            <div className="flex-1 min-w-[200px]">
              <label
                ...
                className="block text-sm font-medium"
              >
                {mounted ? t("room.form.gradePercentage") : "Loading..."}
              </label>
              <input
                id="GradePercentage"
                ...
                type="number"
                onChange={formik.handleChange}
                value={formik.values.GradePercentage}
                className="w-full p-3 border bg-primary-200 text-primary-800 shadow-sm rounded-md"
              />
              {formik.errors.GradePercentage &&
                formik.touched.GradePercentage && (
                  <p className="text-red-500 text-xs">
                    {formik.errors.GradePercentage}
                  </p>
                )}
          ...
          <div>
            <label
              htmlFor="PreviousYearHosting"
              className="block text-sm font-medium"
            >
              {mounted ? t("room.form.previousYearHosting") : "Loading..."}
            </label>
            <select
              id="PreviousYearHosting"
              name="PreviousYearHosting"
              onChange={(e) =>
                formik.setFieldValue(
                  "PreviousYearHosting",
                  e.target.value === "true"
              ...
              value={formik.values.PreviousYearHosting.toString()}
              className="w-[618px] p-3 border bg-primary-200 text-primary-800 shadow-sm rounded-md"
            >
              <option value="true">
                {mounted ? t("choice.yes") : "Loading..."}
            ...
            {formik.errors.PreviousYearHosting &&
              formik.touched.PreviousYearHosting && (
                <p className="text-red-500 text-xs">
                  {formik.errors.PreviousYearHosting}
                </p>
              )}
...
export default ReservationFormOld;
